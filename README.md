...
Фронт перепакован под туристический discovery-flow:

- первый вход открывает онбординг: город, формат поездки, вайбы, бюджет и спальни;
- главная механика — свайп вправо/влево, но просмотренные объявления не пропадают, а получают более низкий приоритет;
- отдельная вкладка «Контакты» убрана: контакт, FB-ссылка и сообщение открываются в «Шортлисте» после сохранения;
- добавлен экран «Радар поездки» со статистикой новых/просмотренных/сохранённых и сбросом seen-приоритета;
- карточка стала короче, фото и описание не конфликтуют со свайпом, полный контент уехал в bottom sheet;
- Telegram WebApp SDK используется мягко: haptics/openLink работают при наличии Telegram и не ломают web-preview.
